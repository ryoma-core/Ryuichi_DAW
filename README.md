...
## 1. 어떤 것인지 설명
**Ryuichi DAW**는 샘플 기반 작곡을 위한 미니 DAW(Digital Audio Workstation)입니다.  
C++(JUCE)로 UI/호스트를 구성하고, Rust(DLL) 오디오 엔진을 FFI로 연동하여 **실시간 출력/믹싱/VST3**를 처리합니다.

### 핵심 기능
- Digital Audio Workstation (DAW 샘플 작곡 프로그램)
- 실시간 출력(Realtime Audio)
- 볼륨/뮤트/팬 파라미터
- VST3 플러그인 로딩/적용
- 디코딩(Symphonia) + Lock-free RingBuffer(rtrb) 기반 파이프라인

---

## 2. 목차
- [1. 어떤 것인지 설명](#1-어떤-것인지-설명)
- [3. 게임소개](#3-게임소개)
- [4. 프로젝트 개요](#4-프로젝트-개요)
- [5. 스크립트 사용된 기술](#5-스크립트-사용된-기술)
- [6. 기술스택](#6-기술스택)
- [7. 참고사항(설치및 사용방법으로)](#7-참고사항설치및-사용방법으로)
...
## 5. 스크립트 사용된 기술
> 여기서는 “어떻게 빌드하고 실행하는지”를 **명령/설정 단위로** 모아둡니다.
